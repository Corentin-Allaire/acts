...
if "__main__" == __name__:
    ...
    matMapDeco = acts.MappingMaterialDecorator(
        tGeometry=trackingGeometry, level=acts.logging.VERBOSE
    )
    ...
    for key in binDict:
        experiments[key] = build_experiment(
            "s_" + str(key), version="1", space=space, storage=storage
        )
    ...
    for trial in range(args.numberOfTrials):
        ...
        scores = runMaterialMappingVariance(
            binMap, args.topNumberOfEvents, refID, pathExp
        )
        for key in binDict:
            objective = 0
            ...
            results = [dict(name="surface_score", type="objective", value=objective)]
