...
def PrepareData(data : pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    ...
    input = data.drop(columns=[target_column, 'track_id', 'nMajorityHits', 'nSharedHits', 'truthMatchProbability', 'Hits_ID', 'chi2', 'pT', 'cluster'])
    # Normalise variables
...
def ClusterTracks(event : pd.DataFrame, DBSCAN_eps : float=0.07, DBSCAN_min_samples : int=2) -> pd.DataFrame :
    ...
    @return: DataFrame identical to the output with an added collum with the cluster 
    """
...
def subClustering(clusterarray : np.ndarray, c : int, lastCluster : float) -> np.ndarray :
    ...
    if(newCluster >= c+1):
        raise RuntimeError("Too many subcluster in the clusters, this shouldn't be possible.")
    hits_IDs = []
...
CKF_files = sorted(glob.glob("itk_output" + "/event0000000[0-9][0-9]-tracks_ckf.csv"))
...
for clusteredEvent in clusteredData:
    ...
    idx = cleanedEvent.groupby(['cluster'])['score'].transform(max) == cleanedEvent['score']
...
for clusteredEvent, cleanedEvent in zip(clusteredData, cleanedData):
    nb_part += clusteredEvent.loc[clusteredEvent['good/duplicate/fake'] != 'fake'].index.nunique()
    nb_track += clusteredEvent.shape[0]
    nb_fake += clusteredEvent.loc[clusteredEvent['good/duplicate/fake'] == 'fake'].shape[0]
    nb_duplicate += clusteredEvent.loc[clusteredEvent['good/duplicate/fake'] == 'duplicate'].shape[0]
    
    nb_good_match += cleanedEvent.loc[cleanedEvent['good/duplicate/fake'] == 'good'].shape[0]
    nb_reco_fake += cleanedEvent.loc[cleanedEvent['good/duplicate/fake'] == 'fake'].shape[0]
    nb_reco_duplicate += cleanedEvent.loc[cleanedEvent['good/duplicate/fake'] == 'duplicate'].shape[0]
    nb_reco_part += cleanedEvent.loc[cleanedEvent['good/duplicate/fake'] != 'fake'].index.nunique()
...
print("duplicate rate: ", 100*((nb_good_match+nb_reco_duplicate)-nb_reco_part)/nb_reco_track, " %")
