...
def PrepareDataSet(CKS_files : list[str]) -> pd.DataFrame :
    ...
    for f in CKS_files:
        ...
        datafile.loc[datafile['good/duplicate/fake'] == 'fake', 'good/duplicate/fake'] = 'duplicate'
...
def PrepareData(data : pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    ...
    input = data.drop(columns=[target_column, 'track_id', 'nMajorityHits', 'nSharedHits', 'truthMatchProbability', 'Hits_ID', 'chi2', 'pT'])
...
def ComputeLoss(score_good : torch.Tensor, score_duplicate : list[torch.Tensor], batch_loss : torch.Tensor, margin: float=0.05) -> torch.Tensor:
...
def train(duplicateClassifier : DuplicateClassifier, data : tuple[np.ndarray, np.ndarray, np.ndarray], epochs : int=20, batch : int=32, validation : float=0.3) -> DuplicateClassifier:
    """Training of the MLP"""
    """
...
CKF_files = sorted(glob.glob("itk_output" + "/event0000000[0-7][0-9]-tracks_ckf.csv"))
...
duplicateClassifier = nn.Sequential(
    ...
    DuplicateClassifier(input_dim, layers_dim)
)
...
torch.onnx.export(duplicateClassifier, input_test, "duplicateClassifier.onnx", input_names=["x"], output_names=["y"], dynamic_axes={'x': {0: 'batch_size'}, 'y': {0: 'batch_size'}})
#==================================================================

# ttbar events for the test
CKF_files_test = sorted(glob.glob("itk_output" + "/event0000000[8-9][0-9]-tracks_ckf.csv"))
...
for sample_test, sample_predict, sample_true in zip(test.index[0:100], output_predict[0:100], y_test[0:100]):
