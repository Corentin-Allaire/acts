...
CKF_files_track = sorted(glob.glob("itk_output_H" + "/event0000000[0-9][0-9]-tracks_ckf.csv"))

CKF_files_resolved = sorted(glob.glob("itk_output"+"/event0000000[0-9][0-9]-tracks_ambiML.csv"))
# CKF_files_resolved = sorted(glob.glob("itk_output"+"/event0000000[0-9][0-9]-tracks_ambi.csv"))
...
nb_track_primary = 0
nb_fake_primary = 0
nb_duplicate_primary = 0

nb_good_match_primary = 0
nb_reco_part_primary = 0
nb_reco_fake_primary = 0
nb_reco_duplicate_primary = 0
nb_reco_track_primary = 0


#Compute the different efficiencies
for trackEvent, resolvedEvent in zip(data_track, data_resolved):
    trackEvent = trackEvent[trackEvent.index.str.split('|').str[0] == '1']
    resolvedEvent = resolvedEvent[resolvedEvent.index.str.split('|').str[0] == '1']
    nb_part_primary += trackEvent.loc[trackEvent['good/duplicate/fake'] == 'good'].shape[0]
    nb_track_primary += trackEvent.shape[0]
    nb_fake_primary += trackEvent.loc[trackEvent['good/duplicate/fake'] == 'fake'].shape[0]
    nb_duplicate_primary += trackEvent.loc[trackEvent['good/duplicate/fake'] == 'duplicate'].shape[0]
    
    #Merge two dataFrames and add indicator column
    merged_primary = pd.merge(trackEvent.loc[trackEvent['good/duplicate/fake'] == 'good'], resolvedEvent, on=['particleId', 'nStates', 'nMeasurements', 'nOutliers', 'nHoles', 'ndf', 'chi2/ndf', 'good/duplicate/fake'], how='left', indicator='exists')
    #Add column to show if each row in first DataFrame exists in second
    merged_primary['exists'] = np.where(merged_primary.exists == 'both', True, False)
    merged_primary.to_csv(path_or_buf='merged.csv')
    
    nb_good_match_primary += merged_primary.loc[merged_primary['exists'] == True].shape[0]
    nb_reco_fake_primary += resolvedEvent.loc[resolvedEvent['good/duplicate/fake'] == 'fake'].shape[0]
    nb_reco_duplicate_primary += resolvedEvent.loc[resolvedEvent['good/duplicate/fake'] == 'duplicate'].shape[0]
    nb_reco_part_primary += resolvedEvent.loc[resolvedEvent['good/duplicate/fake'] != 'fake'].index.nunique()
    nb_reco_track_primary += resolvedEvent.shape[0]


    
print("===Initial efficiencies primary particle===")
print("nb particles : ", nb_part_primary)
print("nb track : ", nb_track_primary)
print("duplicate rate: ", 100*nb_duplicate_primary/nb_track_primary, " %")
print("Fake rate: ", 100*nb_fake_primary/nb_track_primary, " %")

print("===computed efficiencies primary particle===")
print("nb particles : ", nb_part_primary)
print("nb good match : ", nb_good_match_primary)
print("nb particle reco : ", nb_reco_part_primary)
print("nb track reco : ", nb_reco_track_primary)
print("Efficiency (good track) : ", 100*nb_good_match_primary/nb_part_primary, " %")
print("Efficiency (particle reco) : ", 100*nb_reco_part_primary/nb_part_primary, " %")
print("duplicate rate: ", 100*nb_reco_duplicate_primary/nb_reco_track_primary, " %")
print("Fake rate: ", 100*nb_reco_fake_primary/nb_reco_track_primary, " %")
